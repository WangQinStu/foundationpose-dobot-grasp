...
def SDFR_adam(sdf_net, trg_pts, obj_scale, init_pose=None, gt_pose=None,
              refine_scale=False, initial_scale=None, sample_farthest=False,
              return_history=False):  # <--- 新增：开关参数，默认为False
    ...
    if init_pose is None:
        init_pose = np.diag(np.ones(4))
        if init_pose.ndim == 2:
            init_pose = init_pose[np.newaxis, ...]
    ...
    pose_history = []
    if return_history:
        init_pose_np = np.eye(4, dtype=np.float32)
        init_pose_np[:3, :3] = so3_exp_map(w).detach().cpu().numpy()[0]
        init_pose_np[:3, 3] = optim_t.detach().cpu().numpy()[0]
        pose_history.append(init_pose_np)
    ...
    for _itr in range(100):
        ...
        # ========== 新增：只有在需要时才记录 ==========
        if return_history:
            current_pose_np = np.eye(4, dtype=np.float32)
            current_R = so3_exp_map(w).detach().cpu().numpy()[0]
            current_t = optim_t.detach().cpu().numpy()[0]
            current_pose_np[:3, :3] = current_R
            current_pose_np[:3, 3] = current_t
            pose_history.append(current_pose_np)

    # ========== 修改：根据开关决定返回值 ==========
    if return_history:
        if refine_scale:
            return so3_exp_map(w).detach().cpu(), optim_t.detach().cpu(), optim_z.detach().cpu(), pose_history
        else:
            return so3_exp_map(w).detach().cpu(), optim_t.detach().cpu(), None, pose_history
    else:
        # 保持原来的返回逻辑，不影响 eval_SDFR 运行
        if refine_scale:
