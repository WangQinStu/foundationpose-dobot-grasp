import open3d as o3d
import os
from scipy.spatial.transform import Rotation 
import numpy as np
import json
from tqdm import tqdm
import torch
from lib import rendering
import glob
from sdfr import *


import math
import argparse
import glob
import logging as log
log.basicConfig(format='[%(asctime)s] [INFO] %(message)s', 
                datefmt='%d/%m %H:%M:%S',
                level=log.INFO)
from lib.trainer import Trainer
from lib.options import parse_options

def init_SDFR(args,obj_mesh_file):

    sdfr_model=obj_mesh_file.replace('.obj','.pth')
    if not os.path.exists(sdfr_model):
        args.net='OctreeSDF'
        args.num_lods=5
        args.dataset_path=obj_mesh_file
        args.epochs=40
        args.model_path='/'.join(sdfr_model.split('/')[:-1])
        args.exp_name=sdfr_model.split('/')[-1][:-4]
        log.info(f'Training on {args.dataset_path}')
        model = Trainer(args, '')
        model.train()

    obj_mesh, _,_ = rendering.load_object(obj_mesh_file, resize=False, recenter=False)
    obj_mesh.rescale(0.001)
    V = torch.tensor(obj_mesh.vertices * 1000,dtype=torch.float32)
    V_max, _ = torch.max(V, dim=0)
    V_min, _ = torch.min(V, dim=0)
    V_center = (V_max + V_min) / 2.
    V = V - V_center
    max_dist = torch.sqrt(torch.max(torch.sum(V ** 2, dim=-1)))
    obj_scale = 1. / max_dist
    scale_info=obj_mesh_file.replace('.obj','_scale.txt')
    np.savetxt(scale_info,np.array([obj_scale,*V_center]))

def run_SDFR(args,sdf_net,center_scale_info,data_loader,result_file,refine_scale=False,sample_farthest=True):

    mode=args.mode
    if mode=='noise': sample_farthest=False
    if mode=='scale': refine_scale=True

    obj_scale,center_x,center_y,center_z=center_scale_info
    V_center=torch.tensor([center_x,center_y,center_z],dtype=torch.float32).cuda()
    eps=1e-6
    record={}
    pred_pose_list=[]
    R_error_list=[]
    t_error_list=[]
    for src_pts_file,trg_pts_file,src_pts,trg_pts,init_pose,gt_pose in tqdm(data_loader):
        # import pdb;pdb.set_trace()
        if refine_scale:
            initial_scale=0.5*torch.norm(trg_pts.max(dim=1)[0]-trg_pts.min(dim=1)[0],dim=1)/torch.norm(src_pts.max(dim=1)[0]-src_pts.min(dim=1)[0],dim=1)
            initial_scale=initial_scale.cuda()
        else:
            initial_scale=None

        R,t,z=SDFR_adam(sdf_net,trg_pts,obj_scale,init_pose,gt_pose,refine_scale=refine_scale,initial_scale=initial_scale,sample_farthest=sample_farthest)
        r_loss=(torch.einsum('bii->b',torch.bmm(R,gt_pose[:,:3,:3].permute(0,2,1)))-1)/2
        r_loss=torch.clip(r_loss,min=-1+eps,max=1-eps)
        R_error=torch.arccos(r_loss)*180/np.pi
        t_error=(t-gt_pose[:,:3,3]).norm(dim=1)
        # print('R_error:{},t_error:{}'.format(R_error.mean(),t_error.mean()))
        R_error_list.extend(R_error.numpy().tolist())
        t_error_list.extend(t_error.numpy().tolist())

        pred_pose=np.zeros((R.shape[0],4,4))
        pred_pose[:,3,3]=1
        pred_pose[:,:3,:3]=R.detach().cpu().numpy()
        pred_pose[:,:3,3]=t.detach().cpu().numpy()
        if refine_scale:
            pred_pose[:,3,3]=z.detach().cpu().numpy().squeeze()
        pred_pose_list.extend(pred_pose.tolist())


    record['pred_pose_list']=pred_pose_list
    record['5deg,5cm']=(((np.array(R_error_list)<5) & (np.array(t_error_list)<0.05)).sum()).tolist()

    with open(result_file,'w') as f:
        json.dump(record,f,indent=2)


# ========== 极简兼容版：迭代过程动画可视化函数 ==========
def draw_optimization_animation(source_mesh, target_pcd, pose_history,
                                frame_interval=50, loop_play=True):
    """
    绘制SDFR优化过程的动画 (极简兼容版)
    """
    import open3d as o3d
    import time
    import numpy as np

    # 1. 准备数据
    target_temp = o3d.geometry.PointCloud(target_pcd)
    target_temp.paint_uniform_color([1, 0, 0])  # 红色：Target

    # 源模型采样
    source_pcd_orig = source_mesh.sample_points_uniformly(number_of_points=5000)
    source_pcd_orig.paint_uniform_color([0, 0, 1])  # 蓝色：Source
    original_points = np.asarray(source_pcd_orig.points).copy()

    # 用于显示的点云
    source_pcd_display = o3d.geometry.PointCloud(source_pcd_orig)

    # 坐标系
    coordinate_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1)

    # 2. 初始化可视化 (使用最简单的 draw_geometries 回调机制，或者非阻塞)
    # 这里使用最稳妥的非阻塞方式，但不修改标题
    vis = o3d.visualization.Visualizer()
    vis.create_window()

    vis.add_geometry(target_temp)
    vis.add_geometry(source_pcd_display)
    vis.add_geometry(coordinate_frame)

    total_iters = len(pose_history)
    print(f"动画开始：总迭代步数 {total_iters - 1}")
    print("请在弹出的窗口中观看动画，按ESC可退出")

    # 3. 动画循环
    running = True
    while running:
        for iter_idx, pose in enumerate(pose_history):
            # 手动计算变换
            pts = original_points.copy()
            pts_homo = np.hstack([pts, np.ones((pts.shape[0], 1))])
            pts_transformed = (pose @ pts_homo.T).T[:, :3]

            # 更新点云位置
            source_pcd_display.points = o3d.utility.Vector3dVector(pts_transformed)

            # 更新渲染
            vis.update_geometry(source_pcd_display)
            if not vis.poll_events():
                running = False
                break
            vis.update_renderer()

            time.sleep(frame_interval / 1000)

            # 终端打印进度
            if iter_idx % 10 == 0:
                print(f"  迭代进度: {iter_idx}/{total_iters - 1}", end='\r')

        if not loop_play or not running:
            break

    vis.destroy_window()
    print("\n动画结束。")


from torch.utils.data import Dataset,DataLoader

class Visual_Dataset(Dataset):
    def __init__(self,data_dir,render=False):
        self.data_dir=data_dir

        self.source_file_dir=os.path.join(data_dir,'source')
        self.target_file_dir=os.path.join(data_dir,'target')
        self.target_file_list=glob.glob(os.path.join(data_dir,'target/*.ply'))

        gt_file_list_path=os.path.join(data_dir,'gt_file_list.json')
        if os.path.exists(gt_file_list_path):
            self.gt_file_list=json.load(open(gt_file_list_path))
        else:
            self.gt_file_list=[i.replace('target','gt_pose').replace('model_','').replace('.ply','.txt') for i in self.target_file_list]
            with open(gt_file_list_path,'w') as f: json.dump(self.gt_file_list,f)
        
        self.render=render
        
    def __getitem__(self, index):


        model_no=self.gt_file_list[index].split('/')[-1].split('_')[-1].split('.')[0]
        target_file=os.path.join(self.target_file_dir,f'model_{model_no}.ply')
        if self.render:
            source_file=os.path.join(self.source_file_dir,f'model_render_{model_no}.ply')
        else:
            source_file=os.path.join(self.source_file_dir,f'model_{model_no}.ply')

        trg_pts=np.array(o3d.io.read_point_cloud(target_file).points)
        src_pts=np.array(o3d.io.read_point_cloud(source_file).points)

        trg_pts=trg_pts[np.random.choice(np.arange(trg_pts.shape[0]),5000,replace=True)]
        src_pts=src_pts[np.random.choice(np.arange(src_pts.shape[0]),5000,replace=True)]

        gt_pose=np.loadtxt(os.path.join(self.data_dir,f'gt_pose/{model_no}.txt'))
        init_pose=np.loadtxt(os.path.join(self.data_dir,f'init_pose/{model_no}.txt'))
        
        return source_file,target_file,src_pts.astype(np.float32),trg_pts.astype(np.float32),init_pose.astype(np.float32),gt_pose.astype(np.float32)

    def __len__(self):
        return len(self.gt_file_list)

def eval_SDFR(args):
    root_dir=args.root_dir
    mode=args.mode
    results_path=args.results_path
    model_path_list=glob.glob(os.path.join(root_dir,'models/*/*.obj'))

    if mode=='noise': batch_size=32
    else: batch_size=160

    for model_path in model_path_list:
        # if mode=='standard' or mode=='noise' or mode=='scale':
        #     if 'lm' not in model_path and 'ycbv' not in model_path and 'tless' not in model_path:
        #         continue

        init_SDFR(args, model_path)
        sdf_net=get_sdf_net(args,model_path.replace('.obj','.pth'))
        center_scale_info=np.loadtxt(model_path.replace('.obj','_scale.txt'))

        render_path=os.path.join(root_dir,f'render/{mode}','_'.join(model_path.split('/')[-2:])[:-4])
        result_path='{}/sdfr/{}/{}'.format(results_path,mode,'_'.join(model_path.split('/')[-2:])[:-4])
        os.makedirs(result_path,exist_ok=True)
        for _dir in os.listdir(render_path):
            result_file=os.path.join('{}/{}.json'.format(result_path,_dir))
            if os.path.exists(result_file): continue
            dataset_dir=os.path.join(render_path,_dir)
            dataset=Visual_Dataset(dataset_dir)
            dataloader=DataLoader(dataset,batch_size=batch_size,shuffle=False,drop_last=False)
            run_SDFR(args,sdf_net,center_scale_info,dataloader,result_file)


if __name__ == '__main__':
    parser = parse_options(return_parser=True)
    parser.add_argument('--root-dir', type=str, default='./datasets', help='Root dir of datasets')
    parser.add_argument('--results-path', type=str, default='./results', help='Results dir')
    parser.add_argument('--mode', type=str, default='standard',
                        choices=['standard', 'noise', 'scale', 'diverse', 'corrupt'])
    parser.add_argument('--gpu', type=int, default=0)

    # ===================== 修复：换个不冲突的参数名 =====================
    # python run_sdfr.py --vis-debug --target-obj /home/qin-desktop-5060/workspace/SDFR/datasets/models/lm/obj_000001.obj
    parser.add_argument('--vis-debug', action='store_true', help='开启可视化调试模式')
    parser.add_argument('--target-obj', type=str, default=None,
                        help='指定调试物体路径，例如: ./datasets/models/lm/obj_000001.obj')
    parser.add_argument('--sample-idx', type=int, default=0,
                        help='指定调试第几个样本')
    args = parser.parse_args()

    # ===================== 调试模式：只跑一个物体 =====================
    if args.vis_debug and args.target_obj is not None:
        print(f"🔍 调试模式：专门分析物体 {args.target_obj}")

        # 1. 初始化模型（和原代码完全一致）
        init_SDFR(args, args.target_obj)
        sdf_net = get_sdf_net(args, args.target_obj.replace('.obj', '.pth'))
        center_scale_info = np.loadtxt(args.target_obj.replace('.obj', '_scale.txt'))
        obj_scale, center_x, center_y, center_z = center_scale_info

        # 2. 构造数据集路径（和原代码完全一致）
        model_name = '_'.join(args.target_obj.split('/')[-2:])[:-4]
        render_path = os.path.join(args.root_dir, f'render/{args.mode}', model_name)

        # 3. 只取第一个文件夹
        _dir = sorted(os.listdir(render_path))[0]
        dataset_dir = os.path.join(render_path, _dir)
        dataset = Visual_Dataset(dataset_dir)

        # 4. 取指定样本
        idx = args.sample_idx
        src_file, trg_file, src_pts, trg_pts, init_pose, gt_pose = dataset[idx]

        # 5. 转 Tensor 并加 batch 维度（和原代码完全一致）
        src_pts = torch.from_numpy(src_pts).unsqueeze(0).cuda()
        trg_pts = torch.from_numpy(trg_pts).unsqueeze(0).cuda()
        init_pose = torch.from_numpy(init_pose).unsqueeze(0).cuda()
        gt_pose = torch.from_numpy(gt_pose).unsqueeze(0).cuda()

        # 6. 运行推理，获取位姿历史（核心修改）
        print(f"🚀 开始推理样本 {idx}，记录迭代位姿...")
        R_pred, t_pred, _, pose_history = SDFR_adam(
            sdf_net, trg_pts, center_scale_info[0],
            init_pose, gt_pose,
            refine_scale=False,
            sample_farthest=True,
            return_history=True  # <--- 必须加上这个，才会返回 pose_history
        )

        # ========== 新增：加载原始模型 ==========
        print("加载模型，准备动画...")
        obj_mesh = o3d.io.read_triangle_mesh(args.target_obj)

        obj_scale, center_x, center_y, center_z = center_scale_info
        V_center_from_file = np.array([center_x, center_y, center_z])
        V_orig = np.array(obj_mesh.vertices)
        V_processed = (V_orig - V_center_from_file) / 1000.0
        obj_mesh.vertices = o3d.utility.Vector3dVector(V_processed)

        target_pcd = o3d.io.read_point_cloud(trg_file)
        # ==============================
        # 🧪 Step 6.5：坐标系正确性验证（非常关键）
        # ==============================
        print("🧪 验证 Pred 是否真正对齐 target ...")

        model_pts = np.asarray(obj_mesh.sample_points_uniformly(10000).points)


        def transform(pts, R, t):
            return (R @ pts.T).T + t.reshape(1, 3)


        pts_pred = transform(model_pts, R_pred.cpu().numpy(), t_pred.cpu().numpy())

        target_pts = np.asarray(target_pcd.points)

        # 最近邻距离（ADD-S风格快速检查）
        from scipy.spatial import KDTree

        tree = KDTree(target_pts)
        dist, _ = tree.query(pts_pred)

        print("📏 Pred → Target 平均误差 (m):", dist.mean())
        print("📏 Pred → Target 中位误差 (m):", np.median(dist))
        # ===================== 【修正版：点云级误差验证】 =====================

        # 1. 构造预测位姿
        pred_pose = np.eye(4, dtype=np.float32)
        pred_pose[:3, :3] = R_pred.numpy()
        pred_pose[:3, 3] = t_pred.numpy()

        # 2. GT pose
        gt_pose_np = gt_pose.squeeze().cpu().numpy()

        # 3. 采样模型点云
        model_pts = np.asarray(obj_mesh.sample_points_uniformly(10000).points)


        # 4. 分别用 Pred 和 GT 变换模型点云
        def transform_points(pts, pose):
            pts_homo = np.hstack([pts, np.ones((pts.shape[0], 1))])
            pts_trans = (pose @ pts_homo.T).T[:, :3]
            return pts_trans


        pts_pred = transform_points(model_pts, pred_pose)
        # === 修正GT到centered坐标系 ===
        V_center = V_center_from_file / 1000.0  # ⚠️ 注意单位

        gt_pose_fixed = gt_pose_np.copy()
        gt_pose_fixed[:3, 3] = gt_pose_np[:3, 3] + gt_pose_np[:3, :3] @ V_center

        pts_gt = transform_points(model_pts, gt_pose_fixed)
        pts_target = np.asarray(target_pcd.points)

        # 5. 计算 ADD(-S) 风格的误差 (点云到点云的距离)
        # 这才是位姿估计领域的黄金标准
        from scipy.spatial import KDTree

        # 计算 Pred 到 Target 的距离 (视觉上重合，这个应该很小)
        tree_pred = KDTree(pts_pred)
        dists_pred_to_target, _ = tree_pred.query(pts_target)
        mean_dist_pred = dists_pred_to_target.mean()

        # 计算 GT 到 Target 的距离 (如果 GT 定义不一致，这个会很大)
        tree_gt = KDTree(pts_gt)
        dists_gt_to_target, _ = tree_gt.query(pts_target)
        mean_dist_gt = dists_gt_to_target.mean()

        tree_gt_model = KDTree(pts_gt)
        dists_pred_to_gt, _ = tree_gt_model.query(pts_pred)
        mean_dist_pred2gt = dists_pred_to_gt.mean()

        print("\n" + "=" * 50)
        print("✅ 【点云级验证结果】 (不依赖位姿定义)")
        print("=" * 50)
        print(f"🔵 预测模型 到 🔴 观测点云 的平均距离: {mean_dist_pred * 1000:.2f} mm")
        print(f"🟢 GT 位姿 到 🔴 观测点云 的平均距离:   {mean_dist_gt * 1000:.2f} mm")
        print(f"🟦 预测模型 到 🟢 GT 模型 的平均距离:    {mean_dist_pred2gt * 1000:.2f} mm")
        print("=" * 50)
        if mean_dist_pred < 5.0:  # 5mm 以内认为非常准
            print("🎉 结论：预测结果非常准确！(视觉重合是对的)")
            print("   GT 位姿文件可能存在坐标系定义差异，无需在意。")
        else:
            print("请检查数据")
        print("=" * 50 + "\n")
        # 把模型用 pred pose 变换

        # ===================== 可视化确认 =====================
        print("正在打开可视化窗口确认...")
        pcd_red = o3d.geometry.PointCloud(target_pcd)
        pcd_red.paint_uniform_color([1, 0, 0])

        pcd_blue = o3d.geometry.PointCloud()
        pcd_blue.points = o3d.utility.Vector3dVector(pts_pred)
        pcd_blue.paint_uniform_color([0, 0, 1])

        pcd_green = o3d.geometry.PointCloud()
        pcd_green.points = o3d.utility.Vector3dVector(pts_gt)
        pcd_green.paint_uniform_color([0, 1, 0])

        coord = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1)
        o3d.visualization.draw_geometries([pcd_red, pcd_blue, coord])
        print("V_center (meter):", V_center)
        print("\n🎉 调试结束！")
        exit()

    # ===================== 原来的全量评估 =====================
    eval_SDFR(args)

