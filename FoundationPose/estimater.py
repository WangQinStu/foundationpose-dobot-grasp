...
class FoundationPose:
  ...
  def get_tf_from_centered_mesh(self):
    tf_from_center = torch.eye(4, dtype=torch.float, device='cuda')
    tf_from_center[:3,3] = torch.as_tensor(self.model_center, device='cuda', dtype=torch.float)
    return tf_from_center


  def centered_pose_to_full(self, pose):
    tf = self.get_tf_to_centered_mesh()
    if torch.is_tensor(pose):
      return pose@tf
    return pose@tf.data.cpu().numpy()


  def full_pose_to_centered(self, pose):
    tf = self.get_tf_from_centered_mesh()
    if torch.is_tensor(pose):
      return pose@tf
    return pose@tf.data.cpu().numpy()


  def set_pose_last_from_full_pose(self, pose):
    centered_pose = self.full_pose_to_centered(pose)
    self.pose_last = torch.as_tensor(centered_pose, device='cuda', dtype=torch.float)
    return self.pose_last
