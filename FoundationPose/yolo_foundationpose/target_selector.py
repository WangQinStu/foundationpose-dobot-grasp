...
class BottleTargetSelector:
  def __init__(self, target_cls_id=None, min_mask_area=1000, min_valid_depth_ratio=0.15,
               ...
               max_interference_ratio=0.22, prefer_foreground=True,
               foreground_depth_window=0.06):
    ...
    self.foreground_depth_window = foreground_depth_window
  ...
  def _foreground_candidates(self, candidates):
    if not self.prefer_foreground or len(candidates) <= 1:
      return candidates

    depths = [
      cand.score_detail.get('median_depth')
      for cand in candidates
      if cand.score_detail.get('median_depth') is not None
    ]
    if len(depths) == 0:
      return candidates

    front_depth = min(depths)
    depth_limit = front_depth + max(0.0, float(self.foreground_depth_window))
    foreground = [
      cand
      for cand in candidates
      if cand.score_detail.get('median_depth') is not None
      and cand.score_detail['median_depth'] <= depth_limit
    ]
    if len(foreground) == 0:
      return candidates

    foreground_indices = {cand.index for cand in foreground}
    for cand in candidates:
      median_depth = cand.score_detail.get('median_depth')
      cand.score_detail['foreground_front_depth'] = front_depth
      cand.score_detail['foreground_depth_limit'] = depth_limit
      cand.score_detail['foreground_selected'] = cand.index in foreground_indices
      if median_depth is not None:
        cand.score_detail['foreground_depth_delta'] = float(median_depth - front_depth)
    return foreground
  ...
  def select_best(self, result, image, depth=None):
    ...
    selectable = self._foreground_candidates(candidates)
    out = self.make_selection(max(selectable, key=lambda x: x.score), image, depth=depth)
